...
export function insertImport(source: ts.SourceFile, fileToEdit: string, symbolName: string, fileName: string, isDefault = false): Change {
  ...
  const useStrict = findNodes(rootNode, ts.isStringLiteral as any).filter((n: any) => n.text === 'use strict');
  let fallbackPos = 0;
...
export function addSymbolToNgModuleMetadata(
  ...
  if (matchingProperties.length == 0) {
    ...
    if (importPath !== null) {
      return [new InsertChange(ngModulePath, position, toInsert), insertImport(source as any, ngModulePath, symbolName.replace(/\..*$/, ''), importPath)];
    } else {
