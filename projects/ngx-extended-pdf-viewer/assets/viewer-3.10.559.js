...
var _modalsManager = __webpack_require__(45);
...
const PDFViewerApplication = exports.PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    if (appConfig.modalsManger) {
      this.modalsManger = new _modalsManager.ModalsManger(appConfig.modalsManger, eventBus, l10n, container);
    }
  },
  async run(config) {
  ...
  openSignaturesTool() {
    this.eventBus.dispatch("opensignaturetoolmodal", {
      source: this
    });
  },
  ...
  bindEvents() {
    ...
    eventBus._on("signaturetoolchanged", webViewerOpenSignaturesTool);
  ...
  unbindEvents() {
    ...
    eventBus._off("signaturetoolchanged", webViewerOpenSignaturesTool);
  ...
  if (cmd === 0) {
    ...
    switch (evt.keyCode) {
      ...
      case 27:
        ...
        eventBus.dispatch("closesignaturetoolmodal", {
          source: window
        });
        break;
      case 40:
...
function webViewerOpenSignaturesTool() {
  PDFViewerApplication.openSignaturesTool();
}
...
class AnnotationEditorParams {
  ...
  #bindListeners({
    ...
    const dispatchEvent = (typeStr, value) => {
      ...
      this.eventBus.dispatch("beforeswitchannotationeditorparams", eventArgs);
      this.eventBus.dispatch("switchannotationeditorparams", eventArgs);
    };
    ...
    if (editorStampSelectSignatures && editorStampSelectSignatures.length > 0) {
      for (let index = 0; index < editorStampSelectSignatures.length; index++) {
        const element = editorStampSelectSignatures[index];
        element.addEventListener("click", e => {
          const img = e.currentTarget.querySelector("img");
          if (img) {
            createImageBitmap(img).then(bitMap => {
              dispatchEvent("CREATE", bitMap);
            }).catch(e => console.error("something wrong went create image bitmap!!!", e));
          }
        });
      }
    }
...
class Toolbar {
  ...
  constructor(options, eventBus, l10n) {
    ...
    this.buttons = [{
      ...
      element: options.signatureTool,
      eventName: "signaturetoolchanged"
    }, {
      element: options.signaturesFreeTextTool,
      eventName: "switchannotationeditormode",
      eventDetails: {
        get mode() {
          const {
            classList
          } = options.signaturesFreeTextTool;
          return classList.contains("toggled") ? _pdfjsLib.AnnotationEditorType.NONE : _pdfjsLib.AnnotationEditorType.FREETEXT;
        }
      }
    }, {
      element: options.signaturesInkTool,
      eventName: "switchannotationeditormode",
      eventDetails: {
        get mode() {
          const {
            classList
          } = options.signaturesInkTool;
          return classList.contains("toggled") ? _pdfjsLib.AnnotationEditorType.NONE : _pdfjsLib.AnnotationEditorType.INK;
        }
      }
    }];
  ...
  #bindEditorToolsListener({
    ...
    signaturesFreeTextTool,
    signaturesFreeTextParamsTool,
    signaturesInkTool,
    signaturesInkParamsTool
  }) {
    const editorModeChanged = ({
      ...
      signaturesFreeTextTool && (0, _ui_utils.toggleCheckedBtn)(signaturesFreeTextTool, mode === _pdfjsLib.AnnotationEditorType.FREETEXT, signaturesFreeTextParamsTool);
      signaturesInkTool && (0, _ui_utils.toggleCheckedBtn)(signaturesInkTool, mode === _pdfjsLib.AnnotationEditorType.INK, signaturesInkParamsTool);
      const isDisable = mode === _pdfjsLib.AnnotationEditorType.DISABLE;
      editorFreeTextButton.disabled = isDisable;
      editorInkButton.disabled = isDisable;
      editorStampButton.disabled = isDisable;
      signaturesInkTool && (signaturesInkTool.disabled = isDisable);
      signaturesFreeTextTool && (signaturesFreeTextTool.disabled = isDisable);
    };
...
exports.ModalsManger = void 0;
class ModalsManger {
  constructor(options, eventBus, l10n, container) {
    this.eventBus = eventBus;
    this.l10n = l10n;
    this.container = container;
    this.modals = [{
      modal: {
        element: options["signaturesModal"]["modal"],
        eventName: "opensignaturetoolmodal"
      },
      closeModal: {
        element: options["signaturesModal"]["close"],
        eventName: "closesignaturetoolmodal"
      }
    }];
    this.#bindListeners();
  }
  #bindListeners() {
    for (const {
      ...
      if (!modal || !closeModal) continue;
      const _onOpenModal = ({
        source
      }) => {
        modal.element && (modal.element.style.display = "block");
        this.container && (this.container.style.overflow = "hidden");
      };
      const _onCloseModal = () => {
        modal.element && (modal.element.style.display = "none");
        this.container && (this.container.style.overflow = "auto");
      };
      if (modal.eventName !== null) this.eventBus.on(modal.eventName, _onOpenModal.bind(this));
      if (closeModal.eventName !== null) this.eventBus.on(closeModal.eventName, _onCloseModal.bind(this));
      window.onclick = event => {
        if (event.target === modal.element) this.eventBus.dispatch(closeModal.eventName, {
          source: this
        });
      };
      closeModal.element.addEventListener("click", evt => {
        if (closeModal.eventName !== null) {
          this.eventBus.dispatch(closeModal.eventName, {
            source: this
          });
        }
      });
...
exports.ModalsManger = ModalsManger;
...
function getViewerConfiguration() {
  return {
    ...
    annotationEditorParams: {
      ...
      modalsManger: {
        signaturesModal: {
          modal: document.querySelector("#signatures_modal"),
          close: document.querySelector(".signatures_modal-close")
        }
      }
    },
